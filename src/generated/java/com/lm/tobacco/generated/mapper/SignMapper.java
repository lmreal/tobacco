...
import static com.lm.tobacco.generated.mapper.SignDynamicSqlSupport.*;
...
public interface SignMapper {
    ...
    default int insert(Sign record) {
        return insert(SqlBuilder.insert(record)
                .into(sign)
                .map(emplId).toProperty("emplId")
                .map(address).toProperty("address")
                .map(date).toProperty("date")
                .map(photoName).toProperty("photoName")
                .build()
    ...
    default int insertSelective(Sign record) {
        return insert(SqlBuilder.insert(record)
                .into(sign)
                .map(emplId).toPropertyWhenPresent("emplId", record::getEmplId)
                .map(address).toPropertyWhenPresent("address", record::getAddress)
                .map(date).toPropertyWhenPresent("date", record::getDate)
                .map(photoName).toPropertyWhenPresent("photoName", record::getPhotoName)
                .build()
                .render(RenderingStrategy.MYBATIS3));
    }
    ...
    default QueryExpressionDSL<MyBatis3SelectModelAdapter<List<Sign>>> selectDistinctByExample() {
        return SelectDSL.selectDistinctWithMapper(this::selectMany, id, emplId, address, date, photoName)
                .from(sign);
    }
