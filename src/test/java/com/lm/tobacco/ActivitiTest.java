...
public class ActivitiTest {
    ...
    public void currentHandler() {
        List<ProcessInstance> processInstances = processEngine.getRuntimeService().createProcessInstanceQuery().active().variableValueEquals("employeeName", "张三").list();
        //.variableValueEquals("employeeName", "张三")
        System.out.println("流程实例：" + processEngine.getHistoryService().createHistoricTaskInstanceQuery().processUnfinished().taskAssignee("张三").list().get(0).getDescription());
        System.out.println("流程实例：" + processEngine.getRuntimeService().createProcessInstanceQuery().involvedUser("张三").list().get(0));
        String id = processEngine.getRuntimeService().createProcessInstanceQuery().involvedUser("张三").list().get(0).getId();
        TaskService taskService = processEngine.getTaskService();
        System.out.println(taskService.createTaskQuery().count());
        for (ProcessInstance p : processInstances) {
            Task task = taskService.createTaskQuery().active().processInstanceId(p.getProcessInstanceId()).list().get(0);
            System.out.println("任务名：" + task.getName() + "  内容：" + task.getDescription() + " 流程当前处理人:" + task.getAssignee() + task.toString());
        }
    }
